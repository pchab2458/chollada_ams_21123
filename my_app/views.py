...
def billing(request):
    # --- Check fo OPEN Bills --------------------------------------------------------
    open_bill = Billing.objects.filter(status='open')
    if open_bill:
        if len(open_bill) > 1:
...
def create_bill(room_no):
    ...
    room_acc_cost = exd['Bed'] + exd['Dressing Table'] \
...
def update_pf_and_bill(roomno, cd):
    ...
    pay_day = bill.payment_date.day  # int
...
def delete_user(request, rmn):
    ...
    bd = tprofile.bill_date
    try:
        tb = get_object_or_404(Billing, room_no=rmn, bill_date=bd)
        b_status = tb.status
        if b_status == 'close':
            user.delete()
            messages.success(request, 'User {} has been deleted!!!'.format(user))
        else:
            messages.error(request, 'Cannot delete, Bill is Open !!!')
...
def maintenance_charge(request):
    if request.method == 'POST':
        ...
        if maintenance_form.is_valid():
            ...
            new_ma_charge.description = 'Maintenance cost'
